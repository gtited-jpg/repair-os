...
const App: React.FC = () => {
  ...
  // ----------------------------------------------------------------
  // Bulk data loader for signed-in users
  // ----------------------------------------------------------------
  const fetchAppData = async (loggedInUser: Employee) => {
    ...
    try {
      ...
      setError(err.message || 'Failed loading application data.');
      setAppState('error');
    }
  };
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (event === 'SIGNED_IN' && session) {
        setAppState('loading');
        try {
          // Look up employee profile for this email
          ...
          if (employeeProfile) {
            ...
            // Maybe a brand-new user who just completed email confirmation?
            ...
            if (user && user.user_metadata?.organization_name) {
              ...
              // No profile and not a just-created admin → deny
              await api.signOut();
              setError('Access denied. Your email is not registered with an employee profile. Contact your administrator.');
              setAppDisplay('login');
              setAppState('ready');
            }
          }
  ...
  // ----------------------------------------------------------------
  useEffect(() => {
    const toRemove = allThemes.map(t => `theme-${t.toLowerCase()}`);
    document.body.classList.remove(...toRemove);
    const cls = `theme-${activeTheme.toLowerCase()}`;
  ...
  // ----------------------------------------------------------------
  // Auth handlers (keep UI identical; fix Google redirect)
  // ----------------------------------------------------------------
  const handleLogin = async (email: string, pass: string) => {
    ...
    try {
      ...
      if (!user) {
        // If login returns null and user metadata doesn't signal "new org" flow, it's invalid creds.
  ...
  // Force redirect to the current origin (localhost:3000 during dev; Vercel in prod)
  const handleGoogleLogin = async () => {
    setError(null);
    try {
      const redirectTo = window.location.origin; // <-- keeps you on localhost when running locally
      await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo,
          queryParams: {
            // Optional: prompt=select_account helps switch accounts cleanly
  ...
  // ----------------------------------------------------------------
  // View renderers (UI stays identical)
  // ----------------------------------------------------------------
  const renderView = () => {
    ...
    switch (currentView) {
      ...
      case 'tickets':
        return <TicketsView currentUser={currentUser} />;
      case 'inventory':
        return <InventoryView currentUser={currentUser} />;
      case 'pos':
        return (
          <PointOfSaleView
      ...
      case 'vendors':
        return <VendorsView currentUser={currentUser} />;
      case 'payroll':
        return <PayrollView />;
      case 'analytics':
        return (
      ...
      case 'promotions':
        return <PromotionsView addLogEntry={addLogEntry} currentUser={currentUser} />;
      case 'messaging':
      ...
      case 'login':
        return (
          <LoginView
            ...
            onShowPortal={() => setAppDisplay('customer_portal')}
            onShowSignUp={() => setAppDisplay('signup')}
            loginError={error}
          />
        );
      case 'signup':
        return (
          <SignUpView
            ...
            onShowLogin={() => setAppDisplay('login')}
          />
        );
      case 'customer_portal':
        return (
          <CustomerPortalView
            onGoToLogin={() => setAppDisplay('login')}
            logoUrl={logoUrl}
          />
        );
      case 'app':
            ...
            return (
              <div className="flex items-center justify-center w-full h-full">
                <div className="text-center max-w-2xl p-8 bg-dc-panel border border-red-500/50 rounded-lg">
                  ...
                  <p className="mt-2 text-dc-text-secondary">
                    Could not load application data from Supabase.
                  </p>
